...
public class MapEditor : Editor
{
    ...
    public MapManager m_CurrentMapManager;
    // Variable Edit Mode
    ...
    private BrushState m_CurrentEditBrushState = BrushState.One;
    private bool m_CurrentEditEdgeState = false;
    ...
    public override void OnInspectorGUI()
    {
        ...
        if(m_IsInEditSquareMode)
        {
            ...
            m_CurrentEditBrushState = (BrushState)EditorGUILayout.EnumPopup(m_CurrentEditBrushState);
        }
        ...
        if (m_IsInEditEdgeMode)
        {
            ...
            // Affichage Bouton d'edition du state d'une Edge de la map
            m_CurrentEditEdgeState = GUILayout.Toggle(m_CurrentEditEdgeState, "Add/Remove Edge");
        }
    ...
    private void OnSceneGUI()
    {
        ...
        if (m_CanEditMouseAndKeyConstraints)
        {
            ...
            if (m_IsInEditEdgeMode)
            {
                ...
                DisplayGizmoEditEdgeInScenet(intersectPosInt);
                EditCurrentEdgeState(intersectPosInt);
            }
        }
    }
    ...
    private void DisplayGizmoEditEdgeInScenet(Vector3 position)
    {
        // Affichage du gizmo uniquement si on est dans la grille
        if (TestIfPositionIsInLimit(position))
        {
            DrawEdgeSelectedGizmo(position);
    ...
    private void DrawEdgeSelectedGizmo(Vector3 intersectPos)
    {
    ...
    #region EDIT SQUARE
    private void EditCurrentEdgeState(Vector3 intersectPosInt)
    {
        if (Event.current.button == 0)
        {
            switch (Event.current.type)
            {
                ...
                case EventType.MouseDrag:
                    int index = m_CurrentMapManager.GetIndexEdgesFromPos(intersectPosInt);
